...
=> it doesn't support indexing or slicing.. but
=>written using curly brackets

"""
duplicate_set={1,2,3,3,4}
print(duplicate_set) #this will remove all the duplicates

"""
EMPTY SET -> created using set() function
          =>not using {} -> this will create a dictionary
"""
len(duplicate_set)#4


"""
since set can't be indexed ..
...
#HOMEWORK
#frozenset and set difference ..

"""
METHODS
    =>add() - adds a single elements
    =>update() - adds multiple elements from iterable it will just append those members of that iterable to the set.
"""
nums_set={1,2,4}
print("AFTER ADD")
nums_set.update("57")
...
"""
if element already exists and if we
 - use add() function then it won't work..
 won't throw error.
"""

"""
Important
- remove(value) ->remove element ,  raises  key error if not found
- discard(value) -> removes element,  does nothing if not found
- pop() -> removes and returns a 'random element'
- clear() -> removes all elements
- del  -> deletes the entire set ..
        -> since del can support slicing
        -> and also indexing here it can't do it
 'in' operatirs checks if element exists
 -> it is very fast ..O(1)
 -> more efficient than checking in lists
...
- Difference :elements in first set but not in the second set
- Symmetric Difference:elements in either set but not bothg
"""
#IMPORTANT
#U CAN JUST BASICALLY PASS MULTIPLE PARAMETERS
set_a={1,2,3,4,5}
set_b={4,5,6,7,8}
...
#Difference
 # its just elements in first set but not in second
#.difference
...
An object is hashable if:
    ...
    - It has a hash value (a fixed integer returned by a hash() function)
    - That hash value never changes during the object’s lifetime
    - It can be compared for equality with other objects
    hash(10)          # ✔ int
    hash(3.14)        # ✔ float


What does “non-hashable” mean?

    - An object is non-hashable if:
        - It does not have a stable hash value
        - Or its value can change over time
        - These objects cannot be used as dictionary keys or set elements.
        - Most non-hashable objects are mutable.


"""

"""
sorted() -> always gives list as output not like sets...
"""
"""
OPERATORS
- ASSIGNMENT
